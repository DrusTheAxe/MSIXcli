...
private:
    ...
    {
        return m_package.CertificateIssuer();
    }

    FILETIME CertificateValidFrom() const
    {
        return m_package.CertificateValidFrom();
    }

    FILETIME CertificateValidTo() const
    {
        return m_package.CertificateValidTo();
    }

    MSIX::Signing::CertificateValid CertificateValidStatus() const
    {
...
private:
    ...
    // Current certificate dialog status-badge colors, applied by the owner-draw handler (DrawStatusBadge).
    StatusColor m_certificateStatusColor{ StatusColor::None };
};
